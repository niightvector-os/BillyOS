...
export default function Core() {
  ...
  return (
    <>
      {signupPromptOpen && (
        <div className="signup-modal-overlay" onClick={closeSignupPrompt}>
          <div className="signup-modal-card" onClick={(e) => e.stopPropagation()}>
            <button className="signup-modal-close" onClick={closeSignupPrompt} aria-label="Close">×</button>
            <img src="/favicons/logo-mark-64.png" alt="BillyOS" className="signup-modal-logo" />
            <h2>Sign up or log in to continue using BillyOS and enjoy the chat</h2>
            <p>You've used up your free trial messages. Create a free account to keep chatting with no limits.</p>
            <a href="/login" className="signup-modal-google">
            ...
            <a href="/login" className="signup-modal-email">Sign up with email instead</a>
          </div>
        </div>
      )}
      {idle && (
